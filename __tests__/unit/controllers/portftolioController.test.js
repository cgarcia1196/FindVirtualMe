...
describe('GET /portfolio/me', () => {
  it('should return portfolio if found', async () => {
    ...
    const res = await request(app).get('/portfolio/me').query({ email: 'test@example.com' });
  ...
  it('should return 404 if portfolio not found', async () => {
    const res = await request(app).get('/portfolio/me').query({ email: 'notfound@example.com' });
